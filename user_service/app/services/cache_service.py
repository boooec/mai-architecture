...
class UserCacheService:
    ...
    @classmethod
    async def set_user(cls, user: User):
        ...
        await redis_client.set(key, json.dumps(user_data), ex=60 * 60)

    @classmethod
    async def delete_user(cls, login: str):
        key = cls._user_cache_key(login)
    ...
    async def set_search_users(cls, first_name: str, last_name: str, users: list):
        key = cls._search_cache_key(first_name, last_name)
        await redis_client.set(key, json.dumps(users), ex=60 * 60)
