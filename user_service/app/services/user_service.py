...
class UserService:
    ...
    async def register_user(self, session: AsyncSession, login: str, password: str, first_name: str, last_name: str,) -> User:
        ...
        await UserCacheService.set_user(user)
        return user

    async def get_user_by_login(self, session: AsyncSession, login: str) -> Optional[User]:
        cached = await UserCacheService.get_user(login)
        if cached:
            return User(**cached)
    ...
    async def find_users_by_name(self, session: AsyncSession, first_name: str, last_name: str) -> List[User]:
    ...
    async def update_user_by_login(self, session: AsyncSession, login: str, full_name: Optional[str] = None,) -> Optional[User]:
        user = await self.repo.get_by_login(session, login)
        if not user:
            return None
        updated_user = await self.repo.update_user(session, user, full_name)
        await UserCacheService.set_user(updated_user)
        return updated_user
